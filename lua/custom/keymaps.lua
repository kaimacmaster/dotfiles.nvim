...
-- Move lines up/down with Alt+j/k (works on Linux, Windows, and macOS)
...
-- Add these to make it work in Visual Mode (for moving multiple lines)
vim.keymap.set('v', '<M-j>', ":move '>+1<CR>gv=gv", { desc = 'Move lines down in visual mode', silent = true })
vim.keymap.set('v', '<M-k>', ":move '<-2<CR>gv=gv", { desc = 'Move lines up in visual mode', silent = true })

-- Move lines up/down with Alt+j/k (macOS)
...
-- Add these to make it work in Visual Mode (for moving multiple lines)
vim.keymap.set('v', '∆', ":move '>+1<CR>gv=gv", { desc = 'Move lines down in visual mode', silent = true })
vim.keymap.set('v', '˚', ":move '<-2<CR>gv=gv", { desc = 'Move lines up in visual mode', silent = true })
